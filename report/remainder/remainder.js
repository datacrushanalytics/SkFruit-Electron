...
async function remainder() {
  ...
  try {
    ...
    const url = isAdmin
      ? "http://94.136.190.129:3000/remainderReport"
      : `http://94.136.190.129:3000/remainderReport/${sessionData[0].route}`;
...
async function sendMessagesToAll(type) {
  ...
  try {
    ...
    Swal.fire({
      ...
      } messages sent successfully to all users.`,
    });
  } catch (error) {
...
async function exportToExcel() {
  try {
    ...
    return fetch("http://94.136.190.129:3000/remainderReport/generate-pdf", {
      method: "POST",
      body: JSON.stringify(data1),
      headers: {
        "Content-Type": "application/json",
