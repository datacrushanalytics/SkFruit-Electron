...
function fetchDataAndProcess() {
  ...
  return fetch("http://192.168.1.19:3000/accountLedger", {
    method: "POST",
...
function populateTable4(data) {
  ...
  data.reports.forEach(function (item) {
    if (item.online_amt !== 0) {
      // Check if online_amt is not zero
...
async function exportToExcel() {
  try {
    ...
    return fetch("http://192.168.1.19:3000/accountLedger/generate-pdf", {
      method: "POST",
      body: JSON.stringify(data),
      headers: {
        "Content-Type": "application/json",
