...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://13.201.94.88/fetchPurchaseid')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            fetch('http://13.201.94.88/purchaseproductData/' + (parseInt(data[0]['num']) + 1))
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
...
// function form2(){
    ...
    await fetch('http://13.201.94.88/purchaseData/insertPurchase', {
        method: 'POST',
        headers: {
